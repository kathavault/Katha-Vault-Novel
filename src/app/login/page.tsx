  ...
  const handleLogin = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (authError.code) {
        ...
        errorMessage = `${authError.message || "An unexpected error occurred during authentication."} If this error persists and mentions 'App Check' or 'token', ensure Firebase App Check enforcement for 'Authentication' is 'Not enforced' in your Firebase Console settings.`;
      }
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (authError.code) {
        switch (authError.code) {
          case 'auth/firebase-app-check-token-is-invalid':
            title = "IMPORTANT: Google Sign-In - App Check Token Invalid";
            errorMessage = "Firebase App Check blocked this Google Sign-In attempt. \n\nACTION REQUIRED: \n1. Go to Firebase Console -> App Check -> Your Web App -> Services. \n2. Set 'Enforcement' for 'Authentication' to 'Not enforced'. \n3. Save changes. (It may take a few minutes to apply). \n\n(Since client-side App Check is removed, this is the only way to resolve this if you don't want App Check).";
            break;
          ...
          case 'auth/account-exists-with-different-credential':
            ...
            break;
          case 'auth/network-request-failed':
            errorMessage = "Network error during Google Sign-In. Check internet. If you still see 'App Check token invalid' errors after checking console settings, this network error might be related to App Check enforcement being ON in your Firebase Console for 'Authentication' and your reCAPTCHA key (if previously configured) not being valid for this domain or not working.";
            break;
          ...
          case 'auth/operation-not-allowed':
             ...
             break;
          default:
            errorMessage = `Google Sign-In Error: ${authError.message || 'An unexpected error occurred.'} (Code: ${authError.code}). If this error persists and mentions 'App Check' or 'token', ensure Firebase App Check enforcement for 'Authentication' is 'Not enforced' in your Firebase Console settings.`;
        }
      } else {
        errorMessage = `${authError.message || "An unexpected error occurred during Google Sign-In."} If this error persists and mentions 'App Check' or 'token', ensure Firebase App Check enforcement for 'Authentication' is 'Not enforced' in your Firebase Console settings.`;
      }
      toast({ title: title, description: errorMessage, variant: "destructive", duration: 15000 });
    } finally {
      setIsGoogleSubmitting(false);
